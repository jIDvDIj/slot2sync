...
| [`build-game-titles.mjs`](./build-game-titles.mjs) | Gera `src-tauri/assets/game-titles.tsv` (serial → nome do jogo) a partir dos DATs do [libretro-database](https://github.com/libretro/libretro-database)              | `node scripts/build-game-titles.mjs` — quando quiser atualizar os nomes          |
