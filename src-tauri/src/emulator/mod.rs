...
mod tests {
    ...
    #[test]
    fn detecta_retroarch_em_pasta_de_dados() {
        let tmp = tempfile::tempdir().unwrap();
        mkdirs(tmp.path(), &["saves", "states"]);

        let profile = detect_emulator(tmp.path()).expect("deveria detectar RetroArch");

        assert_eq!(profile.name, "RetroArch");
